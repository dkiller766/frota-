...
export default function Veiculos() {
    ...
    const handleImportExcel = async (e) => {
        ...
        reader.onload = async (event) => {
            try {
                ...
                const vehiclesToImport = rows.slice(1)
                ...
                if (vehiclesToImport.length === 0) {
                    alert('Nenhum dado válido de veículo encontrado após o cabeçalho.');
                } else if (window.confirm(`Deseja importar ${vehiclesToImport.length} veículos?`)) {
                    const result = await bulkAddVehicles(vehiclesToImport);
                    if (result.success) {
    ...
    const handleDelete = (id) => {
        if (window.confirm('Tem certeza que deseja excluir o registro deste veículo da frota?')) {
            deleteVehicle(id);
        }
    };
                ...
                {allowAdd && (
                    <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
                        ...
                        <button className="btn btn-primary" onClick={openAddModal}>
                            <Plus size={18} />
                            <span className="hidden-mobile">Novo Veículo</span>
                        </button>
                    </div>
                )}
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '1.5rem', paddingBottom: '2rem' }}>
                {filteredVehicles.length > 0 ? (
                        ...
                        <div key={v.id} className="glass animate-fade-in" style={{ padding: '1.5rem', borderRadius: '1rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                ...
                                <div style={{ background: 'var(--bg-secondary)', padding: '0.5rem', borderRadius: '0.5rem', border: `1px solid ${getStatusColor(v.status)}` }}>
                                    <Car size={24} color={getStatusColor(v.status)} />
                                </div>
                            </div>
                    ...
                    <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: '4rem', color: 'var(--text-secondary)' }}>
                        ...
                        <p>{searchTerm ? 'Nenhum veículo encontrado para esta busca.' : 'Nenhum veículo cadastrado na frota.'}</p>
                    </div>
                )}
            </div>
                ...
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '1rem' }}>
                    <div className="glass animate-slide-up" style={{ width: '100%', maxWidth: '400px', padding: '2rem', borderRadius: '1rem' }}>
                        ...
                        <form onSubmit={handleSaveVehicle} style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                            ...
                            <div className="input-group">
                                <label className="input-label">Área / Território</label>
                                <input
                                    type="text"
                                    list="areas-list"
                            ...
                            <div className="input-group">
                                <label className="input-label">Pátio/Posto Atual</label>
                            ...
                            <div style={{ display: 'flex', gap: '1rem', marginTop: '1rem' }}>
                                ...
                                <button type="submit" className="btn btn-primary" style={{ flex: 1 }}>{editingVehicle ? 'Salvar Edição' : 'Gravar Veículo'}</button>
                            </div>
                        </form>
                    </div>
                </div>
                ...
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '1rem' }}>
                    <div className="glass animate-slide-up" style={{ width: '100%', maxWidth: '400px', padding: '2rem', borderRadius: '1rem' }}>
                        ...
                        <form onSubmit={handleUpdateKM} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                            <div className="input-group">
                                <label className="input-label">KM Hodômetro</label>
                ...
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '1rem' }}>
                    <div className="glass animate-slide-up" style={{ width: '100%', maxWidth: '400px', padding: '2rem', borderRadius: '1rem' }}>
                        ...
                        <form onSubmit={submitMaintenance} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                            <div className="input-group">
                                <label className="input-label">Motivo da Manutenção ou Revisão</label>
                                <textarea
                                    className="input-field"
                                    rows="3"
                                    placeholder="Ex: Troca de óleo, pastilhas gastas, pneu furado..."
                                    value={mntReason}
...
function DamageCanvas({ value, onChange }) {
    ...
    const carSvgString = `<svg xmlns="http://www.w3.org/2000/svg" width="500" height="350" viewBox="0 0 500 350">
  <style>
    .car-line { fill: none; stroke: #6b7280; stroke-width: 2; stroke-linejoin: round; stroke-linecap: round; }
    .car-fill { fill: #f8fafc; stroke: #6b7280; stroke-width: 2; }
    .text { fill: #6b7280; font-family: sans-serif; font-size: 14px; font-weight: bold; text-anchor: middle; }
  </style>

  <!-- LEFT SIDE (Facing Left) -->
  <g transform="translate(10, 20)">
    <text x="110" y="-5" class="text">Lateral Esquerda</text>
    <path class="car-fill" d="M 200,60 L 200,40 L 180,40 L 140,15 L 70,15 L 30,40 L 10,40 L 10,60 L 30,60 A 18 18 0 0 0 66,60 L 144,60 A 18 18 0 0 0 180,60 Z" />
    <circle class="car-line" cx="48" cy="60" r="14" />
    <circle class="car-line" cx="162" cy="60" r="14" />
    <path class="car-line" d="M 130,17 L 75,17 L 45,38 L 170,38 Z" />
    <line class="car-line" x1="105" y1="17" x2="105" y2="60" />
    <line class="car-line" x1="115" y1="42" x2="125" y2="42" />
    <line class="car-line" x1="85" y1="42" x2="95" y2="42" />
  </g>

  <!-- RIGHT SIDE (Facing Right) -->
  <g transform="translate(10, 130)">
    <text x="110" y="-5" class="text">Lateral Direita</text>
    <path class="car-fill" d="M 10,60 L 10,40 L 30,40 L 70,15 L 140,15 L 180,40 L 200,40 L 200,60 L 180,60 A 18 18 0 0 0 144,60 L 66,60 A 18 18 0 0 0 30,60 Z" />
    <circle class="car-line" cx="48" cy="60" r="14" />
    <circle class="car-line" cx="162" cy="60" r="14" />
    <path class="car-line" d="M 80,17 L 135,17 L 165,38 L 40,38 Z" />
    <line class="car-line" x1="105" y1="17" x2="105" y2="60" />
    <line class="car-line" x1="85" y1="42" x2="95" y2="42" />
    <line class="car-line" x1="115" y1="42" x2="125" y2="42" />
  </g>
  ...
  <g transform="translate(10, 240)">
    <text x="50" y="-5" class="text">Traseira</text>
    <rect class="car-line" x="12" y="60" width="14" height="15" rx="3" />
    <rect class="car-line" x="74" y="60" width="14" height="15" rx="3" />
    <path class="car-fill" d="M 15,70 L 15,40 L 25,15 L 75,15 L 85,40 L 85,70 Z" />
    <path class="car-line" d="M 28,18 L 72,18 L 80,38 L 20,38 Z" />
    <rect class="car-line" x="18" y="45" width="15" height="12" rx="2" />
    <rect class="car-line" x="67" y="45" width="15" height="12" rx="2" />
    <rect class="car-line" x="40" y="55" width="20" height="8" />
    <rect class="car-line" x="4" y="32" width="11" height="6" rx="2" />
    <rect class="car-line" x="85" y="32" width="11" height="6" rx="2" />
  </g>

  <!-- FRONT VIEW -->
  <g transform="translate(130, 240)">
    <text x="50" y="-5" class="text">Frente</text>
    <rect class="car-line" x="12" y="60" width="14" height="15" rx="3" />
    <rect class="car-line" x="74" y="60" width="14" height="15" rx="3" />
    <path class="car-fill" d="M 15,70 L 15,40 L 25,15 L 75,15 L 85,40 L 85,70 Z" />
    <path class="car-line" d="M 28,18 L 72,18 L 80,38 L 20,38 Z" />
    <circle class="car-line" cx="25" cy="50" r="7" />
    <circle class="car-line" cx="75" cy="50" r="7" />
    <rect class="car-line" x="38" y="48" width="24" height="6" />
    <rect class="car-line" x="40" y="60" width="20" height="8" />
    <rect class="car-line" x="4" y="32" width="11" height="6" rx="2" />
    <rect class="car-line" x="85" y="32" width="11" height="6" rx="2" />
  </g>
  ...
  <g transform="translate(280, 20)">
    <text x="60" y="-5" class="text">Teto</text>
    <rect class="car-line" x="15" y="35" width="8" height="25" rx="2" />
    <rect class="car-line" x="15" y="235" width="8" height="25" rx="2" />
    <rect class="car-line" x="97" y="35" width="8" height="25" rx="2" />
    <rect class="car-line" x="97" y="235" width="8" height="25" rx="2" />
    <path class="car-fill" d="M 25,15 L 95,15 L 105,50 L 105,250 L 95,285 L 25,285 L 15,250 L 15,50 Z" />
    <rect class="car-line" x="25" y="55" width="70" height="30" />
    <rect class="car-line" x="25" y="215" width="70" height="25" />
    <rect class="car-line" x="30" y="85" width="60" height="130" />
    <rect class="car-line" x="6" y="65" width="9" height="18" rx="2" />
    <rect class="car-line" x="105" y="65" width="9" height="18" rx="2" />
  </g>
</svg>`;
    ...
    const drawBaseGrid = (ctx, canvas, customValue) => {
        ...
        if (customValue) {
            ...
            img.src = `data:image/svg+xml;charset=utf-8,${encodeURIComponent(carSvgString)}`;
        }
    };
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                            ...
                            <div className="input-group">
                                <label className="input-label" style={{ display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                                    <PenTool size={16} /> Assinatura do Responsável *
                                </label>
